...
const ActivityItem = ({ event, getPlayerName }) => (
  <div className="flex items-center gap-4 p-3 rounded-lg bg-slate-900/50 border border-slate-700">
    ...
    <div className="flex-1">
      {event.type === 'game' ? (
        <Link
          to={createPageUrl(`GameDetails?id=${event._id}`)}
      ...
      <p className="text-sm text-slate-400">
        {event.type === 'game'
          ? `Final Score: ${event.finalScore || 'N/A'}`
...
const RecentActivity = ({ events, getPlayerName }) => {
  return (
    <Card
      className={`${DASHBOARD_COLORS.background.card} ${DASHBOARD_COLORS.background.border} shadow-xl ${DASHBOARD_COLORS.effects.hoverShadow} ${DASHBOARD_COLORS.effects.hoverBorder} ${DASHBOARD_COLORS.effects.transition} backdrop-blur-sm`}
    >
      <CardHeader>
        <CardTitle
          className={`text-xl font-bold ${DASHBOARD_COLORS.text.primary} flex items-center gap-2`}
        >
          <Eye className={`w-6 h-6 ${DASHBOARD_COLORS.text.accent}`} />
          Recent Activity
        </CardTitle>
        <p className={`text-sm ${DASHBOARD_COLORS.text.secondary}`}>Latest games and reports.</p>
      </CardHeader>
      <CardContent>
