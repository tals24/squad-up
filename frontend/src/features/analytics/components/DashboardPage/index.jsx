...
const WeeklyTrainingPlanOverview = ({
  teams,
  trainingSessions,
  sessionDrills,
  drills: allDrills,
  ...
  const { weekStart, weekEnd, weekIdentifier, weekSessions } = useMemo(() => {
    ...
    console.log(
      '🔍 Filtered sessions for this week:',
      sessions.length,
      sessions.map((s) => s.sessionTitle)
    );
  ...
  const dailyDrills = useMemo(() => {
      ...
      try {
        ...
        const drillsForThisSession = relevantSessionDrills
          .filter((sd) => {
            ...
            return (
              (sdSessionId === sessionId || sd.TrainingSessions?.includes(sessionId)) &&
              (sd.drill || sd.Drill)
            );
          })
          .map((sd) => {
            ...
            console.log('  - Drill mapping:', {
              drillId,
              drill: drill?.drillName || drill?.DrillName,
            });
            return drill;
          })
          .filter(Boolean);
    ...
    console.log(
      '  - Final drillsByDay:',
      Object.keys(drillsByDay).map((day) => ({ day, count: drillsByDay[day].length }))
    );
    return drillsByDay;
  ...
  return (
    <Card
      className={`${DASHBOARD_COLORS.background.card} ${DASHBOARD_COLORS.background.border} shadow-xl ${DASHBOARD_COLORS.effects.hoverShadow} ${DASHBOARD_COLORS.effects.hoverBorder} ${DASHBOARD_COLORS.effects.transition} backdrop-blur-sm flex flex-col`}
    >
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle
            className={`text-lg font-bold ${DASHBOARD_COLORS.text.primary} flex items-center gap-2`}
          >
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              className="w-7 h-7 hover:bg-slate-700/50"
            >
        ...
        <p className={`text-sm ${DASHBOARD_COLORS.text.secondary} font-mono`}>
          {format(weekStart, 'MMM d')} - {format(weekEnd, 'MMM d, yyyy')}
        </p>
      </CardHeader>
      <CardContent className="flex-grow">
        <h3 className={`text-sm font-semibold ${DASHBOARD_COLORS.text.secondary} mb-2`}>
            ...
            <div className="grid grid-cols-7 gap-3 min-w-max pb-2">
              {[
                ...
                return (
                  <div
                    key={day}
                    className="bg-slate-700/50 p-3 rounded-lg border border-slate-600 text-center min-w-[80px]"
                  >
                    <h4
                      className={`font-semibold ${DASHBOARD_COLORS.text.primary} text-xs mb-2 uppercase tracking-wider`}
                    >
                      {shortDay}
                    </h4>
        ...
        {Object.keys(dailyDrills).length > 0 ? (
          <Link to={`${createPageUrl('TrainingPlanner')}?weekOffset=${weekOffset}`}>
            <Button
              size="sm"
              variant="outline"
              className="bg-slate-800/70 text-cyan-400 border-cyan-400/50 hover:bg-slate-700/70 hover:text-cyan-400 w-full"
            >
              View Full Calendar
            </Button>
          </Link>
          ...
          <div className="text-center">
            <Link to={createPageUrl('TrainingPlanner')}>
              <Button
                size="sm"
                variant="outline"
                className="bg-slate-800/70 text-green-400 border-green-400/50 hover:bg-slate-700/70 hover:text-green-400"
              >
                Plan This Week
              </Button>
            </Link>
...
const PlayerSpotlight = ({ players, reports }) => {
  ...
  const playerStats = useMemo(() => {
    ...
    const playerReports = reports.filter(
      (report) => report.player && report.player._id === currentPlayer._id
    );
  ...
  return (
    <Card
      className={`${DASHBOARD_COLORS.background.card} ${DASHBOARD_COLORS.background.border} shadow-xl ${DASHBOARD_COLORS.effects.hoverShadow} ${DASHBOARD_COLORS.effects.hoverBorder} ${DASHBOARD_COLORS.effects.transition} backdrop-blur-sm p-4`}
    >
      <CardContent className="p-0">
        <Link
          to={createPageUrl(`Player?id=${currentPlayer._id}`)}
          className={`block ${DASHBOARD_COLORS.background.hover} rounded-lg p-2 transition-colors`}
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className={`font-bold text-lg ${DASHBOARD_COLORS.text.primary}`}>
                {currentPlayer.fullName}
              </p>
              <div className="flex items-center gap-2 text-sm">
                <Badge
                  variant="outline"
                  className={`${getPositionBadgeClasses(currentPlayer.position)} hover:bg-transparent`}
                >
                  {currentPlayer.position}
                </Badge>
                {currentPlayer.dateOfBirth && getPlayerAge(currentPlayer.dateOfBirth) && (
                  <span className={DASHBOARD_COLORS.text.secondary}>
                    Age {getPlayerAge(currentPlayer.dateOfBirth)}
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="px-2">
          <h4 className={`text-sm font-bold ${DASHBOARD_COLORS.text.accent} mb-3 text-center`}>
            Performance Stats
          </h4>
          {playerStats ? (
...
export default function Dashboard() {
  ...
  const { filteredTeams, filteredPlayers, filteredReports, filteredGames, userRole } =
    useDashboardData({
      currentUser,
      users,
      teams,
  ...
  return (
    <div className="p-6 md:p-8 bg-slate-900 min-h-screen text-slate-100 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        {/* Header */}
        <DashboardHeader currentUser={currentUser} roleDisplay={roleDisplay} />

        {/* Top Info Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <GameZone games={filteredGames} />
          <WeeklyTrainingPlanOverview
            teams={filteredTeams}
        ...
        {/* Activity Log */}
        <RecentActivity events={recentEvents} getPlayerName={getPlayerName} />
      </div>
    </div>
  );
}
