...
export default function Analytics() {
  ...
  const getFilteredData = () => {
    ...
    if (currentUser.role === 'Coach') {
      filteredTeams = teams.filter((team) => team.Coach && team.Coach.includes(currentUser.email));
    ...
    if (selectedTeam !== 'all') {
      filteredPlayers = filteredPlayers.filter(
        (player) => player.Team && player.Team.includes(selectedTeam)
      );
    }

    const playerIds = filteredPlayers.map((player) => player.id);
    const filteredReports = reports.filter(
      (report) => report.Player && playerIds.some((id) => report.Player.includes(id))
    );
  ...
  const calculateAnalytics = () => {
    ...
    const totalRating = filteredReports.reduce(
      (sum, report) => sum + (report.GeneralRating || 0),
      0
    );
    ...
    const totalMinutes = filteredReports.reduce(
      (sum, report) => sum + (report.MinutesPlayed || 0),
      0
    );
    ...
    const playerPerformance = filteredPlayers
      .map((player) => {
        const playerReports = filteredReports.filter(
          (report) => report.Player && report.Player.includes(player.id)
        );
        ...
        const avgRating =
          playerReports.reduce((sum, report) => sum + (report.GeneralRating || 0), 0) /
    ...
    const recentReports = filteredReports.filter(
      (report) => report.Date && new Date(report.Date) >= thirtyDaysAgo
    );
    ...
    <PageLayout>
          ...
          currentUser?.role !== 'Coach' && (
            <Select value={selectedTeam} onValueChange={setSelectedTeam}>
              ...
              <SelectContent className="bg-slate-800 border-slate-600 text-white">
                <SelectItem value="all" className="focus:bg-slate-700 hover:bg-slate-700">
                  All Teams
                </SelectItem>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <DataCard hover={true} className="hover:shadow-cyan-500/30 hover:border-cyan-500/50">
          ...
          <div className="text-3xl font-bold text-white mb-1">{analytics.averageRating}/5</div>
        ...
        <DataCard hover={true} className="hover:shadow-cyan-500/30 hover:border-cyan-500/50">
          ...
          <div className="text-3xl font-bold text-white mb-1">{analytics.totalGoals}</div>
          <div className="flex items-center text-sm text-slate-400">
        ...
        <DataCard hover={true} className="hover:shadow-cyan-500/30 hover:border-cyan-500/50">
          ...
          <div className="text-3xl font-bold text-white mb-1">{analytics.totalAssists}</div>
          <div className="flex items-center text-sm text-slate-400">
              ...
              {analytics.topPerformers.length > 0 ? (
                analytics.topPerformers.map((performer, index) => (
                  <div
                    ...
                    className="flex items-center gap-4 p-4 rounded-xl bg-slate-900/50 border border-slate-700 hover:shadow-md transition-all duration-200"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge
                          variant="outline"
                          className="text-xs border-border-custom text-text-primary"
                        >
            ...
            <CardContent>
              <div className="space-y-4">
                {Object.entries(analytics.positionBreakdown).map(([position, count]) => {
                  const total = Object.values(analytics.positionBreakdown).reduce(
                    (a, b) => a + b,
                    0
                  );
          ...
          <Card className="shadow-xl border-border-custom bg-bg-secondary/70 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="text-center">
                <div className="text-4xl font-bold text-white mb-2">{analytics.recentTrends}</div>
