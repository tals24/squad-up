...
import { PlayerProfileCard, PerformanceStatsCard, DevelopmentTimeline } from '../shared';
...
export default function Player() {
  ...
  const gameStats = useMemo(() => {
    ...
    const gamesInSquad = playerRosters.filter(
      (roster) => roster.status === 'Starting Lineup' || roster.status === 'Bench'
    ).length;
  ...
  const stats = useMemo(() => {
    if (playerReports.length === 0)
      return { averageRating: 0, totalGoals: 0, totalAssists: 0, totalMinutes: 0 };
    ...
    const totalMinutes = playerReports.reduce(
      ...
      <div className="p-6 md:p-8 bg-slate-900 min-h-screen flex items-center justify-center">
        <div className="max-w-md mx-auto text-center">
          ...
          <p className="text-slate-400 mb-6">
            The player you're looking for doesn't exist or there was an error loading the data.
          </p>
          ...
          <div className="flex items-center gap-4">
            <Link to={createPageUrl('Players')}>
              <Button
                variant="outline"
                size="icon"
                className="border-slate-700 bg-slate-800/50 text-slate-300 hover:bg-slate-700 hover:border-cyan-500 hover:text-cyan-400 transition-all duration-300 backdrop-blur-sm"
              >
                <ArrowLeft className="w-4 h-4" />
              </Button>
            </Link>
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-slate-100">{player.fullName}</h1>
          ...
          <div className="lg:col-span-2 space-y-8">
            <Card className="shadow-2xl border-slate-700 bg-slate-800/70 backdrop-blur-sm">
              ...
              <CardContent className="text-slate-400 text-sm space-y-2">
                <p>
                  The AI will analyze all scouting and game reports to generate a concise summary of
                  the player's strengths, weaknesses, and potential development areas.
                </p>
            ...
            <DevelopmentTimeline playerReports={playerReports} games={games} playerId={playerId} />
          </div>
        </div>
      </div>
    </div>
