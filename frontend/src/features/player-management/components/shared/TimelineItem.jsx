...
const TimelineItem = ({ report, gameForReport, isLast }) => {
  ...
  return (
    <div className="relative pl-12">
      {!isLast && <div className="absolute left-5 top-12 bottom-0 w-0.5 bg-slate-600"></div>}
      ...
      <div className="bg-slate-700/50 rounded-xl p-4 border border-slate-600 hover:border-cyan-500/30 transition-colors duration-300">
        <div className="flex items-center justify-between mb-3">
          <Badge
            variant="outline"
            className={`${reportType === 'Game Report' ? 'text-cyan-400 border-cyan-500/50 bg-cyan-900/30' : 'text-purple-400 border-purple-500/50 bg-purple-900/30'}`}
          >
            {reportType}
          </Badge>
        ...
        <div className="flex items-center gap-2 mb-4">
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`w-4 h-4 ${i < rating ? 'text-yellow-400 fill-current' : 'text-slate-500'}`}
              />
            ))}
          </div>
          <span className="text-sm font-bold text-slate-100 ml-1">{rating}/5 Rating</span>
        </div>

        {reportType === 'Game Report' && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <StatBox
              value={report.minutesPlayed || 0}
              label="Minutes"
              icon={Clock}
