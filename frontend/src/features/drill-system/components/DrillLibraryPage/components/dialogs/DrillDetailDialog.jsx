      ...
      <DialogContent className="sm:max-w-2xl bg-slate-800/95 border-slate-600 backdrop-blur-sm shadow-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-300">
            {drill?.category} - {displayAgeGroups(drill?.targetAgeGroup)}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-6 pt-4">
          <div className="flex flex-wrap gap-2">
            <span
              className={`inline-flex items-center rounded-full px-3 py-1.5 text-sm font-medium ${getCategoryColor(drill?.category)}`}
            >
              <Tag className="w-4 h-4 mr-2" /> {drill?.category}
            </span>
            <span
              className={`inline-flex items-center rounded-full px-3 py-1.5 text-sm font-medium ${getAgeColor()}`}
            >
          ...
          {drill?.layoutData && (
            <div className="bg-slate-700/50 rounded-lg p-4 border border-slate-600">
              ...
              <Link to={`/DrillDesigner?drillId=${drill._id}&readOnly=true`}>
          ...
          {hasValidVideoLink(drill?.videoLink) && (
            <div className="bg-slate-700/50 rounded-lg p-4 border border-slate-600">
              ...
              {getYouTubeVideoId(drill.videoLink) ? (
                ...
                <div
                  onClick={() =>
                    window.open(drill.videoLink, '_blank', 'noopener,noreferrer')
                  }
