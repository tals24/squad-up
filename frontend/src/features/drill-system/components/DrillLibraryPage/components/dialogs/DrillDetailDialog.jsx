      ...
      <DialogContent className="sm:max-w-2xl bg-slate-800/95 border-slate-600 backdrop-blur-sm shadow-2xl">
        ...
        <div className="space-y-6 pt-4">
          <div className="flex flex-wrap gap-2">
            <span
              className={`inline-flex items-center rounded-full px-3 py-1.5 text-sm font-medium ${getCategoryColor(drill?.category || drill?.Category)}`}
            >
          ...
          {(drill?.videoLink || drill?.VideoLink) && (
            <div className="bg-slate-700/50 rounded-lg p-4 border border-slate-600">
              ...
              {getYouTubeVideoId(drill.videoLink || drill.VideoLink) ? (
                ...
                <div
                  onClick={() =>
                    window.open(drill.videoLink || drill.VideoLink, '_blank', 'noopener,noreferrer')
                  }
