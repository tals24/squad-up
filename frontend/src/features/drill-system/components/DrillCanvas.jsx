...
const DrillCanvas = forwardRef(
  ({ activeTool, initialElements, onDrop, onDragOver, onElementsChange, isReadOnly }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      ...
      setElements: (newElements) => {
        // Existing: Allows parent to set elements
        setElements(newElements);
      },
    }));
    ...
    const drawSoccerField = (ctx, width, height) => {
      ...
      // Right penalty area
      ctx.strokeRect(
        width - 10 - penaltyDepth,
        (height - penaltyWidth) / 2,
        penaltyDepth,
        penaltyWidth
      );
      ...
      // Right goal area
      ctx.strokeRect(
        width - 10 - goalAreaDepth,
        (height - goalAreaWidth) / 2,
        goalAreaDepth,
        goalAreaWidth
      );
    ...
    const drawDoubleArrow = (ctx, fromx, fromy, tox, toy) => {
      ...
      ctx.lineTo(
        arrowHeadX - headlen * Math.cos(angle - Math.PI / 6),
        arrowHeadY - headlen * Math.sin(angle - Math.PI / 6)
      );
      ctx.moveTo(arrowHeadX, arrowHeadY);
      ctx.lineTo(
        arrowHeadX - headlen * Math.cos(angle + Math.PI / 6),
        ...
        if (el.type === 'pass-arrow' || el.type === 'dribble-arrow' || el.type === 'shoot-arrow') {
          ...
          if (el.points && el.points.length >= 2) {
            ...
            const endDist = Math.sqrt(
              (x - el.points[el.points.length - 1].x) ** 2 +
                (y - el.points[el.points.length - 1].y) ** 2
            );
    ...
    const handleMouseMove = (e) => {
      ...
      if (activeTool === 'select' && interactionData) {
        setInteractionData((prev) => ({
          ...prev,
          x: pos.x - prev.offsetX,
          y: pos.y - prev.offsetY,
        }));
      ...
      if (interactionData) {
        if (activeTool === 'select') {
          const newElements = elements.map((el) =>
            el.id === interactionData.id
              ? { ...el, x: interactionData.x, y: interactionData.y }
              : el
          );
          setElements(newElements);
          onElementsChange?.(newElements);
          ...
          if (
            (interactionData.type === 'pass-arrow' || interactionData.type === 'shoot-arrow') &&
            ...
            interactionData.type === 'dribble-arrow' &&
            interactionData.points &&
            interactionData.points.length < 2
          ) {
    ...
    return (
      <div className="w-full h-full relative">
        ...
        {showNumberMenu &&
          hoveredElement && ( // hoveredElement is the ID
            <PlayerNumberMenu
