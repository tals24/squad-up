...
export default function TacticalBoard({
  ...
  console.log('🏟️ TacticalBoard render:', {
    ...
    assignedPlayers: Object.entries(formation)
      .filter(([_, p]) => p !== null)
      .map(([posId, p]) => ({ posId, player: p?.fullName })),
  });
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-green-700 via-green-600 to-green-800">
          ...
          {Object.entries(positions).map(([posId, posData]) => {
            ...
            if (isOccupied) {
              console.log(
                `🎨 Rendering position ${posId} (${posData.label}) at (${posData.x}%, ${posData.y}%) with player:`,
                player?.fullName
              );
            }
              ...
              >
                ...
                {isOccupied ? (
                  <div
                    ...
                    title={
                      isPlayed || isDone
                        ? `Click to view ${player.fullName}'s report`
                        : isScheduled && !isReadOnly
                          ? `Click to reassign position`
                          : ''
                    }
                  >
                    <div
                ...
                {!isOccupied && (
                  <div
                    className="text-white text-[10px] font-semibold text-center drop-shadow-lg"
