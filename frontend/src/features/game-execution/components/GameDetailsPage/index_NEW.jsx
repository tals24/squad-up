...
export default function GameDetails() {
  ...
  // Player reports & match stats state
  ...
  // === SECTION 3: Helper Functions (Minimal) ===
  ...
  const difficultyHandlers = useDifficultyHandlers({
    gameId,
    ...
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <TacticalBoardModule
        tacticalBoardProps={{
          ...
          onDragStart: dndHandlers.handleDragStart, onDragEnd: dndHandlers.handleDragEnd, onPositionDrop: dndHandlers.handlePositionDrop,
          onRemovePlayerFromPosition: dndHandlers.handleRemovePlayerFromPosition, onPositionClick: formationHandlers.handlePositionClick,
          isDragging: dndHandlers.isDragging, draggedPlayer: dndHandlers.draggedPlayer, isReadOnly, playersOnPitch, benchPlayers,
        }}
      ...
      <DialogsModule
        dialogsProps={{
          ...
          showCardDialog: dialogState.showCardDialog, selectedCard: dialogState.selectedCard, onSaveCard: cardsHandlers.handleSaveCard,
          ...
          onSaveTeamSummary: reportHandlers.handleTeamSummarySave, onCloseTeamSummaryDialog: () => dialogState.setShowTeamSummaryDialog(false),
          ...
          onClosePlayerSelectionDialog: () => { dialogState.setShowPlayerSelectionDialog(false); dialogState.setSelectedPosition(null); dialogState.setSelectedPositionData(null); },
          showFinalReportDialog: gameStateHandlers.showFinalReportDialog, onConfirmFinalSubmission: gameStateHandlers.handleConfirmFinalSubmission,
          onCloseFinalReportDialog: () => gameStateHandlers.setShowFinalReportDialog(false), isFinalizingGame,
        }}
      />
      <ConfirmationModal
        isOpen={showConfirmationModal} onClose={() => setShowConfirmationModal(false)} onConfirm={confirmationConfig.onConfirm}
        onCancel={confirmationConfig.onCancel} title={confirmationConfig.title} message={confirmationConfig.message}
        confirmText={confirmationConfig.confirmText} cancelText={confirmationConfig.cancelText} type={confirmationConfig.type}
      />
    </div>
  );
}
