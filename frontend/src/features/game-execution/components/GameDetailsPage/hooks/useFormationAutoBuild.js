  ...
  useEffect(() => {
    ...
    if (manualFormationMode) {
      if (hasFormationWithPlayers) {
        console.log(
          '⚠️ [useFormationAutoBuild] Manual formation mode with existing formation - skipping auto-build'
        );
      } else {
        console.log(
          '⚠️ [useFormationAutoBuild] Manual formation mode active (likely restoring from draft) - skipping auto-build'
        );
      }
      return;
    }
    ...
    Object.entries(positions).forEach(([posId, posData]) => {
      ...
      if (matchingPlayer) {
        ...
        console.log(
          `✅ [useFormationAutoBuild - Phase 1] Exact match: ${matchingPlayer.fullName} (${matchingPlayer.position}) → ${posId} (${posData.label})`
        );
      }
    });
    ...
    Object.entries(positions).forEach(([posId, posData]) => {
      ...
      if (matchingPlayer) {
        newFormation[posId] = matchingPlayer;
        console.log(
          `✅ [useFormationAutoBuild - Phase 2] Type match: ${matchingPlayer.fullName} (${matchingPlayer.position}) → ${posId} (${posData.label})`
        );
      }
    });
    ...
    Object.entries(positions).forEach(([posId, posData]) => {
      ...
      if (anyUnplacedPlayer) {
        ...
        console.log(
          `⚠️ [useFormationAutoBuild - Phase 3] Fallback: ${anyUnplacedPlayer.fullName} (${anyUnplacedPlayer.position}) → ${posId} (${posData.label}) [Out of position]`
        );
      } else {
    ...
    console.log(
      `✅ [useFormationAutoBuild] Complete - ${assignedCount}/${totalPositions} positions filled`
    );

    if (assignedCount < startingPlayers.length) {
      console.warn(
        `⚠️ [useFormationAutoBuild] ${startingPlayers.length - assignedCount} starting players could not be placed`
      );
    }
    ...
    if (hasFormationWithPlayers) {
      console.log(
        '🔄 [useFormationAutoBuild] Resetting manual formation mode to allow future auto-builds'
      );
