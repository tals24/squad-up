...
export function useReportDraftManager({
  ...
  useEffect(() => {
    ...
    if (game.reportDraft && typeof game.reportDraft === 'object') {
      ...
      console.log('📋 [useReportDraftManager] Loading report draft:', {
        ...
        playerMatchStatsCount: draft.playerMatchStats ? Object.keys(draft.playerMatchStats).length : 0,
      });
      ...
      if (draft.finalScore) {
        setFinalScore(prev => {
          ...
          console.log('✅ [useReportDraftManager] Merged finalScore:', { prev, draft: draft.finalScore, merged });
          return merged;
        });
      }
      ...
      if (draft.matchDuration) {
        setMatchDuration(prev => {
          ...
          console.log('✅ [useReportDraftManager] Merged matchDuration:', { prev, draft: draft.matchDuration, merged });
          return merged;
        });
      }
    ...
    if (game.status === 'Done') {
      console.log('✅ [useReportDraftManager] Done game - stats will be loaded from PlayerMatchStat collection');
    } else {
      console.log('⚠️ [useReportDraftManager] No draft found, stats will be loaded from PlayerMatchStat collection');
    }
  ...
  const { 
    ...
    shouldSkip: (data) => {
      ...
      const shouldSkip = !hasTeamSummary && !hasFinalScore && !hasMatchDuration && !hasPlayerReports && !hasPlayerMatchStats;
