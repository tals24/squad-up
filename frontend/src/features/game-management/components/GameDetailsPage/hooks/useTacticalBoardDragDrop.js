...
export function useTacticalBoardDragDrop({
  ...
  const executePositionDrop = (player, posId) => {
    console.log(`✅ [useTacticalBoardDragDrop] Assigning player ${player.fullName} to position ${posId}`);
    ...
    setFormation((prev) => {
      ...
      Object.keys(updated).forEach((key) => {
        if (updated[key]?._id === player._id) {
          console.log(`🧹 [useTacticalBoardDragDrop] Removing ${player.fullName} from position ${key}`);
          updated[key] = null;
        }
      });
      ...
      console.log('🔄 [useTacticalBoardDragDrop] Formation updated:', 
