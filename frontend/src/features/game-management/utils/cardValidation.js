 ...
 */
export function canReceiveCard(existingCards, newCardType) {
...
/**
 * Get available card options for a player based on their existing cards
 * 
 * @param {Array} existingCards - Array of existing card objects for the player
 * @returns {{ yellow: boolean, red: boolean, secondYellow: boolean, isSentOff: boolean }}
 */
export function getAvailableCardOptions(existingCards) {
  const playerCards = existingCards || [];
  
  const yellowCount = playerCards.filter(card => card.cardType === 'yellow').length;
  const hasRed = playerCards.some(card => card.cardType === 'red');
  const hasSecondYellow = playerCards.some(card => card.cardType === 'second-yellow');
  const isSentOff = hasRed || hasSecondYellow;
  
  return {
    yellow: !isSentOff && yellowCount === 0,
    red: !isSentOff,
    secondYellow: !isSentOff && yellowCount === 1,
    isSentOff
  };
}

/**
 * Get recommended/default card type for a player based on their existing cards
 * 
 * @param {Array} existingCards - Array of existing card objects for the player
 * @returns {string|null} - Recommended card type or null if no recommendation
 */
export function getRecommendedCardType(existingCards) {
  const playerCards = existingCards || [];
  const yellowCount = playerCards.filter(card => card.cardType === 'yellow').length;
  
  // If player has 1 yellow, recommend second yellow
  if (yellowCount === 1) {
    return 'second-yellow';
  }
  
  // Otherwise, no recommendation (default to yellow)
  return null;
}
