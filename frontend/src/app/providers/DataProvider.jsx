  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!newData.gameReports || newData.gameReports.length === 0) {
        newData.gameReports = (apiData.reports || []).filter(
          (report) => report.reportType === 'GameReport'
        );
      }
  ...
  const fetchOrganizationConfig = async () => {
    try {
      ...
      if (!token) {
        ...
        console.log(
          '🔍 [DEBUG] gameDifficultyAssessmentEnabled in default:',
          defaultConfig.features.gameDifficultyAssessmentEnabled
        );
      ...
      if (result.success && result.data) {
        ...
        console.log(
          ...
          result.data.isDefault ? '(default)' : '(saved)'
        );
        console.log('🔍 [DEBUG] Organization config features:', result.data.features);
        console.log(
          '🔍 [DEBUG] gameDifficultyAssessmentEnabled enabled?',
          result.data.features?.gameDifficultyAssessmentEnabled
        );
      }
    } catch (error) {
      ...
      console.log(
        '🔍 [DEBUG] gameDifficultyAssessmentEnabled in fallback:',
        fallbackConfig.features.gameDifficultyAssessmentEnabled
      );
    } finally {
      setIsLoadingConfig(false);
    }
