...
test.describe('GameDetailsPage Smoke - Scheduled Status', () => {
  ...
  test('should load scheduled game with draft lineup', async ({ page }) => {
    ...
    await expect(
      page.locator('text="Game Day Roster"').or(page.locator('text="Starting Lineup"'))
    ).toBeVisible({ timeout: 5000 });

    // Verify "Game Was Played" button exists (for Scheduled games)
    await expect(
      page.locator('button:has-text("Game Was Played"), button:has-text("Mark as Played")')
    ).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should persist roster changes after autosave', async ({ page }) => {
    ...
    const squadSection = page
      .locator('[data-testid="squad-section"], .squad-section, text=Squad')
      .first();
    ...
    if (await playerDropdown.isVisible({ timeout: 2000 })) {
      ...
      await expect(
        page.locator('[data-testid="starting-lineup"], text=Starting Lineup')
      ).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('should validate starting lineup before transition', async ({ page }) => {
    ...
    const playedButton = page
      .locator('button:has-text("Game Was Played"), button:has-text("Mark as Played")')
      .first();
...
test.describe('GameDetailsPage Smoke - Played Status', () => {
  ...
  test('should load played game with events section', async ({ page }) => {
    ...
    // Verify page loads
    await expect(page.locator('h1, h2, [data-testid="game-header"]')).toBeVisible({
      timeout: 5000,
    });

    // Verify events section visible (Goals, Substitutions, Cards)
    const eventsSection = page
      .locator('[data-testid="events-section"], .events-section, [data-testid="match-analysis"]')
      .first();
    await expect(eventsSection).toBeVisible({ timeout: 5000 });

    // Verify event buttons exist
    const hasGoalButton = await page
      .locator('button:has-text("Goal"), button:has-text("+ Goal")')
      .isVisible({ timeout: 2000 });
    const hasSubButton = await page
      .locator('button:has-text("Substitution"), button:has-text("+ Sub")')
      .isVisible({ timeout: 2000 });
    const hasCardButton = await page
      .locator('button:has-text("Card"), button:has-text("+ Card")')
      .isVisible({ timeout: 2000 });
  ...
  test('should open and close goal dialog', async ({ page }) => {
    ...
    if (await goalButton.isVisible({ timeout: 2000 })) {
      ...
      // Verify dialog opens
      await expect(page.locator('[role="dialog"], .dialog, .modal, text=Goal')).toBeVisible({
        timeout: 3000,
      });
  ...
  test('should open player performance dialog', async ({ page }) => {
    ...
    if (await playerCard.isVisible({ timeout: 3000 })) {
      ...
      await expect(
        page.locator('[role="dialog"]:has-text("Performance"), [role="dialog"]:has-text("Report")')
      ).toBeVisible({ timeout: 3000 });

      // Verify dialog has rating fields
      const hasRatings = await page
        .locator('input[type="number"], input[type="range"], [role="slider"]')
        .isVisible({ timeout: 2000 });
  ...
  test('should show Submit Final Report button', async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Submit Final Report"), button:has-text("Finalize")')
    ).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe('GameDetailsPage Smoke - Done Status', () => {
  ...
  test('should load done game in read-only mode', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="final-score"], .final-score, text=Final:')
    ).toBeVisible({ timeout: 5000 });
  ...
  test('should show finalized player reports', async ({ page }) => {
    ...
    if (await playerCard.isVisible({ timeout: 3000 })) {
      ...
      const editableInputs = await page
        .locator(
          '[role="dialog"] input:not([readonly]):not([disabled]), [role="dialog"] textarea:not([readonly]):not([disabled])'
        )
        .count();
...
test.describe('GameDetailsPage Smoke - Error Handling', () => {
  ...
  test('should handle missing game gracefully', async ({ page }) => {
    ...
    expect(
      foundError || page.url() !== `${BASE_URL}/game-details?id=000000000000000000000000`
    ).toBe(true);
  });
