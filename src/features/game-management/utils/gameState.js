 ...
 */
export function getPlayerStateAtMinute(timeline, playerId, targetMinute, startingLineup = {}, squadPlayers = {}) {
  ...
  for (const event of eventsUpToMinute) {
    ...
    if (event.type === 'substitution') {
      ...
      const playerOutId = event.playerOut?._id?.toString() || 
                         event.playerOutId?.toString() || 
                         event.playerOut?._id || 
                         event.playerOutId || 
                         event.playerOut?.toString();
      const playerInId = event.playerIn?._id?.toString() || 
    ...
    if (event.type === 'card') {
      const cardPlayerId = event.player?._id?.toString() || 
      ...
      if (cardPlayerId?.toString() === normalizedPlayerId && 
 ...
 */
export function filterPlayersByState(players, timeline, minute, startingLineup, squadPlayers, requiredState) {
  if (!minute || minute < 1) {
